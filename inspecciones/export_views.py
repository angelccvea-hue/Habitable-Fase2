...
from inspecciones.workflow import filtrar_casos_por_rol, usuario_puede_ver_caso
...
def export_excel_informe(request, pk: int) -> HttpResponse:
    ...
    data = generar_excel_informe(caso, user=request.user)
...
def export_excel_lote(request) -> HttpResponse:
    ...
    casos = list(
        filtrar_casos_por_rol(request.user, CasoRojo.objects.filter(pk__in=ids)).order_by(
            "-score", "hab_id"
        )
    )
    if not casos:
...
def export_excel_informes_multiples(request) -> HttpResponse:
    ...
    if not casos:
        raise Http404("No hay casos con esos IDs.")
    if len(casos) == 1:
        data = generar_excel_informe(casos[0], user=request.user)
        return _excel_response(data, nombre_archivo_informe(casos[0]))
    data = generar_excel_informes_multiples(casos, user=request.user)
...
def respuesta_excel_admin(request, queryset, modo: str) -> HttpResponse | None:
    ...
    if len(casos) == 1:
        ...
        return _excel_response(
            generar_excel_informe(c, user=request.user),
            nombre_archivo_informe(c),
        )
    if modo == "informe_multi":
        return _excel_response(
            generar_excel_informes_multiples(casos, user=request.user),
